import { reloadWindow, loaderHandler, setLocalStorageInfo, setupUi, createAlert, templateCard, handleLogout } from "./src/js/index.mjs";
  ...
  const response = await axios.get(`${url}/posts?limit=5&page=${current}`)
    ...
    .then(() => {
      ...
      postArray.map((item) => {
        ...
        posts.innerHTML += templateCard(item, conditionEdit, postIdUpdate, id, title, tags);
      });
    })
    .catch((error) => {
...
export const handleClickCard = (e) => {
  ...
  axios
    ...
    .then((response) => {
      ...
      console.log("handle click card handler")
 ...
 */
export const handleAddingComment = (e) => {
 ...
 */
export const handleClickDeleteButton = (e, refresh) => {
  loaderHandler(true);
  const token = localStorage.getItem("token");
  const headers = {
    authorization: `Bearer ${token}`,
 ...
 */
export const showUserInfo = async (element) => {
